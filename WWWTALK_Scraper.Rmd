...
date: "2024-09-27"
Legal_info_WWWTalk: Copyright © 1996-2003 The World Wide Web History Project and Arcady Press.
...
```{r Temporary dataframe with the result of the loop }
...
```{r Import missing link, and merge with its metadata}

Missing_Link<-readLines("Missing_link.txt") #Complete the list by importing the missing link
...
Scraping_df<-rbind(Temporary_df, Missing_Metadata) #Merge missing link's metadata and data with the temporary dataframe
...
```{r Solve problematic date and time}
...
```{r Saving as .rds .txt .xlsx }

#Saving as .rds
saveRDS(Scraping_df, file = "Scraping_df.rds") #Saving as .rds
...
write.xlsx(Directories, file = "Directories.xlsx") #Create an Excel of the Directories
...
ggplot(Timeline, aes(x = Date_Ym, y = Tot_Month)) +             # Create a timeline
  ...
  ggtitle("WWWTALK: Message distribution over time") +          # Title graph
